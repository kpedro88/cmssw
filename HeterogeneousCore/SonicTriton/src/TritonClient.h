...
std::exception_ptr TritonClient<Client>::setup() {
	...
	for(unsigned i0 = 0; i0 < batchSize_; i0++) {
		...
		auto err1 = nicinput_->SetRaw(reinterpret_cast<const uint8_t *>(arr), nInput_ * sizeof(float));
		if (!err1.IsOk()) throw cms::Exception("BadGrpc") << "unable to set data for batch entry " << i0 << ": " << err1;
	}
	auto t3 = std::chrono::high_resolution_clock::now();
	edm::LogInfo("TritonClient") << "Image array time: " << std::chrono::duration_cast<std::chrono::microseconds>(t3-t2).count();
}

template <typename Client>
void TritonClient<Client>::getResults(const std::unique_ptr<nic::InferContext::Result>& result) {
	auto t2 = std::chrono::high_resolution_clock::now();
	...
	auto t3 = std::chrono::high_resolution_clock::now();
	edm::LogInfo("TritonClient") << "Output time: " << std::chrono::duration_cast<std::chrono::microseconds>(t3-t2).count();
}
...
void TritonClient<Client>::evaluate(){
	...
	if (!err.IsOk()) {
		edm::LogWarning("TritonClient") << "Could not read the result" <<  ": " << err;
		this->output_.resize(nOutput_ * batchSize_, 0.f);
	} else {
		auto t3 = std::chrono::high_resolution_clock::now();
		edm::LogInfo("TritonClient") << "Remote time: " << std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count();
...
template <typename Client>
void TritonClient<Client>::reportServerSideState(const ServerSideStats& stats) const
{
	...
	if (cnt == 0)
	{
		edm::LogInfo("TritonClient") << "  Request count: " << cnt;
		return;
	}
	...
	const uint64_t overhead = (cumm_avg_us > queue_avg_us + compute_avg_us)
								  ...
								  : 0;
	edm::LogInfo("TritonClient") << "  Request count: " << cnt << std::endl
			  << "  Avg request latency: " << cumm_avg_us << " usec";

	edm::LogInfo("TritonClient") << " (overhead " << overhead << " usec + "
				<< "queue " << queue_avg_us << " usec + "
				<< "compute " << compute_avg_us << " usec)" << std::endl;
}

template <typename Client>
void TritonClient<Client>::summarizeServerStats(
...
template <typename Client>
void TritonClient<Client>::summarizeServerModelStats(
...
template <typename Client>
void
TritonClient<Client>::getServerSideStatus(
