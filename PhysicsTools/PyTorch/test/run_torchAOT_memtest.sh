...
# 2. Runs the TorchAOTProducer on 1 and 4 threads, both in the default shared
#    GlobalCache mode and in the per-stream control mode.
# 3. Asserts that the shared mode does NOT grow the model .so RSS with threads,
#    while the control mode does -- i.e. weights are shared and read-only.
set -e
...
run() {  # run <nthreads> <perStream> <logfile>
  cmsRun "$CFG" nthreads=$1 nevents=$(( $1 * 30 )) ninfer=1 batch=16 features=10 \
      perStream=$2 model="$REL" > "$3" 2>&1 || { cat "$3"; die "cmsRun nt=$1 perStream=$2"; }
}

echo ">> Shared GlobalCache mode: 1 vs 4 threads"
run 1 0 "$WORK/shared_nt1.log"
run 4 0 "$WORK/shared_nt4.log"
echo ">> Per-stream control mode: 1 vs 4 threads"
run 1 1 "$WORK/ctrl_nt1.log"
run 4 1 "$WORK/ctrl_nt4.log"

s1=$(so_rss "$WORK/shared_nt1.log"); s4=$(so_rss "$WORK/shared_nt4.log")
c1=$(so_rss "$WORK/ctrl_nt1.log");   c4=$(so_rss "$WORK/ctrl_nt4.log")
echo "   shared  .so RSS:  nt1=$s1  nt4=$s4 MB"
echo "   control .so RSS:  nt1=$c1  nt4=$c4 MB"
